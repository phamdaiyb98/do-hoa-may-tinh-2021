...
void bienDoiHLPDV(float tx, float ty, float tz, float sx, float sy, float sz) {
	instance = Translate(tx, ty, tz)*Scale(sx, sy, sz);
	glUniformMatrix4fv(model_view_loc, 1, GL_TRUE, model_view*instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}

void bienDoiHLPDV2(float tx, float ty, float tz, float sx, float sy, float sz, float rz) {
	instance = RotateZ(rz)*Translate(tx, ty, tz)*Scale(sx, sy, sz);
	glUniformMatrix4fv(model_view_loc, 1, GL_TRUE, model_view*instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
...
void base()
{
	bienDoiHLPDV(0, 0, 0, 0.03, 1, 0.03);
}
void upper_arm()
{
	bienDoiHLPDV(0, 0.5, 0.25, 0.03, 0.03, 0.5);
}
void lower_arm()
{
	bienDoiHLPDV2(0, 0.2, 0, 0.1, 0.4, 0.01, 0);
	bienDoiHLPDV2(0, 0.2, 0, 0.1, 0.4, 0.01, 120);
	bienDoiHLPDV2(0, 0.2, 0, 0.1, 0.4, 0.01, -120);
}
...
void Robot_Arm()
{
	...
	base();
	upper_arm();
	model_view = model_view * Translate(0.0, 0.5, 0.5)*RotateZ(theta[2]);
	lower_arm();
}
